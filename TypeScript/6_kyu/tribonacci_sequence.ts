...
export function tribonacci([a, b, c]: [number, number, number], n: number): number[] {
  let trib: number[] = [];
  if(n==0) return trib;	//caso lista vacia
  
  let _i: number = 0;	//counter
  while(_i < n) {
    if(_i < 3)  {	//si son los tres primeros datos, los pusheamos
      switch(_i)  {
          case 0: trib.push(a);
          break;
          case 1: trib.push(b);
    ...
    else	{	//generamos el resto de la secuencia
      let acc: number = 0;
      acc = a + b + c;
      trib.push(acc);
      a=b;
      b=c;
      c=trib[trib.length-1];
      ++_i;
      }
    }
  return trib;
